...
function mark() {
  try {
    ...
    /* private mode: the notice shows again next visit, which is fine */
  }
}

/**
 * One line, shown once. This site sets no cookies itself; Google Maps, which
 * draws the basemap and Street View, may set its own. Saying so once is the
 * honest version of a cookie banner for a site with no tracking of its own.
 *
 * `inline` sits under the landing masthead as part of the page. `fixed` sits
 * along the bottom of the map view, where Google's scripts actually load.
 */
