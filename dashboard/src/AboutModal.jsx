      ...
      <article className="relative my-auto w-full max-w-[38rem] border border-rule-strong bg-paper shadow-paper">
        ...
        <div className="border-b border-rule-strong px-7 py-5">
          ...
          <h2 id="about-title" className="font-serif text-[26px] font-medium leading-[1.15] text-ink">
            {advanced ? "Forward run, 2025–2027" : "Finding dangerous corners before anyone is hurt"}
          </h2>
        </div>
  ...
  return (
    <>
      ...
      <Section heading="What this is">
        <p>
          A ranking of the San Diego intersections below the City&rsquo;s five-crash review line,
          ordered by how likely each is to produce a serious crash. It is built from crash records
          the city already collects, 2016 through 2024 — no new data, no cameras, no sensors.
        </p>
        {isCombined && (
          <p>
            The list on the map also includes {known.toLocaleString()} intersections that have
            already had a serious crash
            {screen > 0 && <> and {screen.toLocaleString()} that meet the City&rsquo;s own screening rule</>}.
            Those are records, not predictions; they sit at the top of the list and are marked as such.
          </p>
        )}
      </Section>
      ...
      <Section heading="How to read it">
        ...
        <p>
          ...
          the average number of vehicles a day, where the City has counted that street at or within
          a block of the corner. Dividing crashes a year by the vehicles entering, in millions,
          gives the rate engineers use to compare a busy arterial with a quiet street; a raw count
          alone makes every quiet street look safe.
        </p>
        <p>
          &ldquo;Police have logged&rdquo; counts San Diego Police collision reports filed to that
          intersection since the model&rsquo;s cutoff; most reports are filed to a block address
          instead and are not counted, so the figure is a floor. Whether a corner has signals or
          stop signs comes from OpenStreetMap and changes the advice for turns.
        </p>
        <p>
          ...
          crash. They are general advice. None of them would have prevented any particular crash,
          and they are no substitute for looking at the corner yourself.
        </p>
      </Section>
      ...
      <Section heading="What it is not">
        <p>
          Independent student research. It is not an official City of San Diego hazard assessment.
          A dot here does not mean an intersection is unsafe today, and the absence of one does not
          mean it is safe.
        </p>
      </Section>

      <p className="mt-7 border-t border-rule pt-4 text-[11px] text-ink-3">
        Crash data: SWITRS via TIMS, UC Berkeley SafeTREC. Traffic counts: City of San Diego open
        data. Road network: OpenStreetMap. Publication forthcoming.
      </p>
    </>
  );
}
  ...
  return (
    <>
      <Section heading="Model">
        <p>
          XGBoost Tweedie regression on 20 crash-history features, hyperparameters fixed by nested
          cross-validation and frozen. Trained once on 2016 to 2021 features against 2022 to 2024
          KSI outcomes, then applied to the 2025 to 2027 candidate cohort by predict-only scoring,
          never refit.
        </p>
      </Section>
      ...
      <Section heading="Signals">
        <p>
          ...
          acceleration from a previously stable baseline (changepoint detection). Cumulative volume
          outweighs short-term recency. Built-environment features added a consistent +0.02
          Spearman on the verified run and did not replicate on the 2025 forward run, so the
          deployed model stays crash-history only.
        </p>
        <p>
          The countermeasure prompts in the detail panel read the crash-type features present in a
          site&rsquo;s top signals (left-turn, night, bicycle, pedestrian, broadside counts over the
          feature window) and attach the standard road-user countermeasure for that crash type.
          They are prompts, and they carry no model weight.
        </p>
      </Section>
      ...
      <Section heading="Exposure">
        <p>
          City of San Diego ADT counts (2023 to 2026 file), matched to a site when a count on one of
          its streets names the other street as a segment limit, else the nearest count on the same
          street within 250 m. Entering volume is the sum over matched streets of the most recent
          count; a one-street match is flagged as a ceiling on the rate. Rate per MEV = crashes/yr
          (2016 to 2024) ÷ (entering ADT × 365 / 10⁶). Trend compares 2022 to 2024 against 2016 to
          2021 (rising at ×1.5 and +1/yr).
        </p>
        <p>
          Recent reports: SDPD collision file (City open data), intersection rows only, matched on
          the unordered street pair since 2025-01-01. Control: OSM highway=traffic_signals or
          highway=stop nodes within 25 m of the site.
        </p>
      </Section>
      ...
      <Section heading="Candidate set">
        ...
        {isCombined && (
          <p>
            The exported list is the combined top-{topN}: {known} known-KSI sites (feature-window KSI
            of 1 or more, all spine nodes), then {screen} City-screen sites (5 or more crashes in
            the last feature year), then model predictions. Tiers are stacked in that order
            (src/export/combined_list.py). The catch statistics above score the model ranking alone.
          </p>
        )}
      </Section>
...
function Plain() {
  ...
  return (
    <>
      ...
      <Section heading="What this is">
        <p>
          A ranking of every {CITY.name} intersection with no serious-crash history, ordered by
