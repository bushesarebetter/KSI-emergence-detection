...
var __exports__ = (() => {
  ...
  // src/adapter/helpers/cpu-hotspot-profiler.ts
      ...
      WebGPUTexture = class extends import_core4.Texture {
        ...
        constructor(device, props) {
          ...
          this.view = new WebGPUTextureView(this.device, {
            ...
            arrayLayerCount: this.props.view?.arrayLayerCount ?? (this.dimension !== "3d" ? this.depth : 1)
          });
    ...
    for (const [bufferName, mappings] of Object.entries(groupMappingsByBuffer(logicalMappings))) {
      ...
      for (const [bindingOffset, vertexAttributes] of Array.from(
        ...
        resolvedLayouts.push({
          bufferName,
          bindingOffset,
          minShaderLocation: (0, import_core8.getMinimumAttributeLocation)(
...
"${error.message}"`), this)();
  ...
  // src/adapter/resources/webgpu-render-bundle.ts
...
"${error.message}"`), this)();
        ...
        setBindings(bindings, options) {
          ...
          const bindGroups = (0, import_core10._getDefaultBindGroupFactory)(this.device).getBindGroups(
            this.pipeline,
            bindings,
...
"${error.message}"`),
        ...
        getRenderBundleEncoderDescriptor() {
          ...
          const descriptor = {
            label: this.props.id,
            colorFormats: renderBundleEncoderProps.colorAttachmentFormats.map(
              (format) => format ? getWebGPUTextureFormat(format) : null
            ),
    ...
    "src/adapter/resources/webgpu-compute-pipeline.ts"() {
      ...
      WebGPUComputePipeline = class extends import_core12.ComputePipeline {
        ...
        _getBindGroups(bindings, bindGroupCacheKeys) {
          ...
          return (0, import_core12._getDefaultBindGroupFactory)(this.device).getBindGroups(
            this,
    ...
    "src/adapter/resources/webgpu-vertex-array.ts"() {
      ...
      WebGPUVertexArray = class extends import_core13.VertexArray {
        ...
        getDrawValidationError() {
          ...
          return missingSlots.length ? `WebGPU vertex buffer slot${missingSlots.length === 1 ? "" : "s"} ${missingSlots.join(
            ", "
          )} not set` : null;
        }
        /**
    ...
    "src/adapter/webgpu-canvas-context.ts"() {
      ...
      WebGPUCanvasContext = class extends import_core14.CanvasContext {
        ...
        _configureDevice() {
          ...
          if (requestedConfiguration.toneMapping?.mode === "extended" && !isHighDynamicRangeCanvasConfiguration(acceptedConfiguration)) {
          ...
          this.colorFormat = acceptedConfiguration?.format || configuredConfiguration.format;
          this.colorSpace = acceptedConfiguration?.colorSpace || configuredConfiguration.colorSpace;
          this.toneMapping = acceptedConfiguration?.toneMapping?.mode || configuredConfiguration.toneMapping?.mode;
...
"${error.message}"`), this)();
        ...
        executeBundles(bundles) {
          ...
          this.handle.executeBundles(
            Array.from(bundles, (renderBundle) => renderBundle.handle)
          );
          this.device.popErrorScope((error) => {
            this.device.reportError(
...
"${error.message}"`),
        ...
        getRenderPassDescriptor(framebuffer) {
          ...
          renderPassDescriptor.colorAttachments = framebuffer.colorAttachments.map(
            (colorAttachment, index) => ({
              ...
              resolveTarget: this.props.resolveTargets?.[index] ? this.props.resolveTargets[index].handle : void 0,
              // ...colorAttachment,
    ...
    "src/adapter/resources/webgpu-compute-pass.ts"() {
      ...
      WebGPUComputePass = class extends import_core18.ComputePass {
        ...
        setPipeline(pipeline) {
          ...
          const bindGroups = (0, import_core18._getDefaultBindGroupFactory)(this.device).getBindGroups(
  ...
  // src/adapter/helpers/webgpu-texture-capabilities.ts
  ...
  // ../shadertools/src/lib/shader-assembly/wgsl-binding-scan.ts
  ...
  var WGSL_BINDABLE_VARIABLE_PATTERN, WGSL_BINDING_DECLARATION_SEPARATOR_PATTERN, MODULE_WGSL_BINDING_DECLARATION_REGEXES, WGSL_BINDING_DECLARATION_REGEXES, WGSL_EXPLICIT_BINDING_DECLARATION_REGEXES, WGSL_AUTO_BINDING_DECLARATION_REGEXES;
  var init_wgsl_binding_scan = __esm({
    "../shadertools/src/lib/shader-assembly/wgsl-binding-scan.ts"() {
      "use strict";
      WGSL_BINDABLE_VARIABLE_PATTERN = "(?:var<\\s*(uniform|storage(?:\\s*,\\s*[A-Za-z_][A-Za-z0-9_]*)?)\\s*>|var)\\s+([A-Za-z_][A-Za-z0-9_]*)";
      WGSL_BINDING_DECLARATION_SEPARATOR_PATTERN = "\\s*";
  ...
  // ../shadertools/src/lib/shader-assembly/wgsl-interface-scan.ts
  ...
  function scanWGSLBindings(tokens, braceDepths, aliases) {
    ...
    return bindings.sort(
      (left, right) => left.group - right.group || left.location - right.location || left.name.localeCompare(right.name)
    );
  }
  ...
  function getWGSLStorageTextureBinding(base, resourceType) {
    const match = /^texture_storage_(1d|2d|2d_array|3d)<([A-Za-z0-9_]+),(read|write|read_write)>$/.exec(
      resourceType
    );
    if (!match) {
      return null;
  ...
  function getWGSLTextureBinding(base, resourceType) {
    const sampledTexture = /^texture_(multisampled_)?(1d|2d|2d_array|cube|cube_array|3d)<(f32|i32|u32)>$/.exec(
      resourceType
    );
    if (sampledTexture) {
  ...
  function normalizeWGSLSamplerBindings(bindings) {
    for (const binding of bindings) {
      ...
      const pairedTexture = bindings.find(
        (candidate) => candidate.type === "texture" && candidate.name === textureName && candidate.group === binding.group
      );
  ...
  function getNumericWGSLAttribute(tokens, name) {
    ...
    for (let tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
      ...
      if (value !== void 0 || tokens[tokenIndex + 2]?.value !== "(" || !/^\d+$/.test(tokens[tokenIndex + 3]?.value || "") || tokens[tokenIndex + 4]?.value !== ")") {
        return null;
      }
  ...
  var init_wgsl_interface_scan = __esm({
    "../shadertools/src/lib/shader-assembly/wgsl-interface-scan.ts"() {
      "use strict";
      init_wgsl_binding_scan();
    }
  });

  // ../shadertools/src/wgsl.ts
  var init_wgsl = __esm({
    "../shadertools/src/wgsl.ts"() {
      "use strict";
      init_wgsl_interface_scan();
    }
  });
...
Raw WebGPU error: ${error.message}`, bindGroup)();
  ...
  function formatBindGroupCreationErrorSummary(shaderLayout, bindings, entries, group) {
    ...
    const missingBindings = expectedBindings.filter(
      (bindingSummary) => !providedBindings.some((provided) => provided.location === bindingSummary.location)
    );
  ...
  function getBindGroupEntry(binding, bindingLayout, options, bindingName = "unknown") {
    ...
    if (binding instanceof import_core25.ExternalTexture && !options?.sampler) {
    ...
    "src/adapter/webgpu-device.ts"() {
      ...
      WebGPUDevice = class extends import_core26.Device {
        ...
        constructor(props, device, adapter, adapterInfo) {
          ...
          this.preferredColorFormat = canvasContextProps?.colorFormat ?? navigator.gpu.getPreferredCanvasFormat();
          this.handle = device;
