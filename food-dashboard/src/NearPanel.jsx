...
export function explain(err) {
  ...
  if (/REQUEST_DENIED|not authorized|ApiNotActivated|PERMISSION_DENIED/i.test(msg)) {
    return "Address lookup is not switched on for this site's Google key. In Google Cloud, enable the Places API (New) and the Geocoding API for the project, and add them to the key's API restrictions.";
  }
...
export default function NearPanel({ facilities, filters, onPoint, onSelect, compact = false }) {
  ...
  async function run(e) {
    ...
    try {
      ...
      const shown = facilities.features.filter((f) => passesFilters(f.properties, filters));
      const places = shown
    ...
    <div className={compact ? "" : "px-6 py-5"}>
      {!compact && (
        <div className="mb-3 flex items-baseline justify-between gap-3">
          ...
          <p className="text-[11px] text-ink-3">listed places within {NEAR_M} m</p>
        </div>
      )}
                ...
                return (
                  <li key={p.rank} className="border-b border-rule last:border-b-0">
                    <button onClick={() => onSelect(feature)} className="flex w-full items-baseline gap-3 py-2 text-left hover:bg-paper-sunk">
                      <span className="tnum w-12 shrink-0 text-[11px] text-ink-3">{Math.round(meters / 10) * 10} m</span>
                      <span className="min-w-0 flex-1">
                        <span className="block text-[13px] leading-snug text-ink">{p.name}</span>
                        <span className="block text-[11px] text-ink-3">#{p.rank}{pattern ? `, ${pattern.toLowerCase()}` : ""}</span>
                      </span>
                    </button>
                  </li>
                );
